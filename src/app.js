...
let db= new sqlite3.Database('./db/mcu.db', sqlite3.OPEN_READWRITE /*| sqlite3.OPEN_CREATE*/, (err) => {
...
function createDatabase() {
    var newdb = new sqlite3.Database('./db/mcu.db', (err) => {
        if (err) {
            console.log("Getting error " + err);
            exit(1);
        }
...
function runQueries(db) {
    db.each("PRAGMA foreign_keys = ON;", (err, row) => {
        console.log(row.id + ": " + row.info);
    });
    
    db.each("PRAGMA foreign_keys;", (err, row) => {
        console.log(row);
    });
